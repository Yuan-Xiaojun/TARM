...
for ii =1:num
    ...
    %---low-rank matrix recovery:(partial orthogonal case)---
    perm=randperm(n);
    indexs=perm(1:m);
    sign1=2*(rand(n,1)>0.5)-1;
    sigma=0; %sqrt(10^(-50/10));
    w=sigma*randn(m,1);
    ...
    %---TARM parameters----
    params.mu=0; %0: 1/delta,1: auto tuning,2: fixed
    ...
    %------RGrad------
    tic;
    [~,mse5]=RGrad(b,dim,A,At,r,params,error_function);
...
plot(1:length(mse0),mse0,'-p b',1:length(mse1),mse1,'-. r',1:length(mse2),mse2,'-> g',1:length(mse3),mse3,'-v k',1:length(mse4),mse4,'--o',1:length(mse5),mse5,'-- p','LineWidth',1.5);
xlabel('Iteration');
ylabel('NMSE (dB)');
legend('TARM','LMAFit','RCG','ALPS','NIHT','RGrad');
